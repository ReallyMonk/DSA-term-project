...
from hamiltonian.map_snake.direction import Direc
from hamiltonian.map_snake.point import PointType
from hamiltonian.map_snake.pos import Pos
...
class Snake:
    ...
    def reset(self, reset_map=True):
        ...
        self._bodies = deque(self._init_bodies)  # become a list [x, x, x, ...]
    ...
    def move(self, new_direc=None):
        ...
        old_head_type, new_head_type = self._new_types()  # up down right left
        self._map.point(self.head()).type = old_head_type
        new_head = self.head().adj(self._direc_next)
        self._bodies.appendleft(new_head)  # bodies = body + head, bodies[0] = head
